...
try {
    ...
    console.warn('Calculator logic not found, using fallback functions');
...
function calculateCalculatorValues(calculatorType, calcData, allData) {
    ...
    if (calculateInvestment) {
        ...
        console.log(`calculateCalculatorValues - dataForCalc sample:`, {
            calculator_type: dataForCalc.calculator_type,
            purchase_price: dataForCalc.purchase_price,
            monthly_rent: dataForCalc.monthly_rent
        });
...
function generatePDF(data, images, outputPath, logoPath) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Investment Opportunity page(s) - one for each selected calculator
            // Note: createInvestmentPage handles page creation for multiple calculators internally
            createInvestmentPage(doc, data, logoPath);
            
            // Only add a page if we need to continue (the function will have already added pages for multiple calculators)
            // We need to check if we're at the end of a page or if there's more content
            // For now, always add a page for the next section (Key Information)
            doc.addPage();
